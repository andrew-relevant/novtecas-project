...
export async function POST(request: Request) {
  try {
    ...
    const { honeypot: _h, consent: _c, productId, rating, ...rest } = parsed.data;
    ...
    const res = await fetch(`${strapiUrl.replace(/\/$/, "")}/api/reviews`, {
      method: "POST",
      headers: {
    ...
    if (!res.ok) {
      ...
      console.error("Strapi POST /api/reviews failed", res.status, text);
